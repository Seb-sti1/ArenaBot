...
def fitnessRobot(listOfCommands, visualize=True):
    ...
    #when it encounters a wall, the nearer it is from the corner the better it is
        ...
        if currentLine.intersects(labyrinthe_wall1) : 
            minDistanceFromCorner = min([currentLine.distance(Point(x)) for x in [(wall1["x"], wall1["y"]), (wall1["x"] + wall1["width"], wall1["y"]),
                                        (wall1["x"] + wall1["width"], wall1["y"] + wall1["height"]),
                                        (wall1["x"], wall1["y"] + wall1["height"])]])
            closeToWallMalus += malusGain + minDistanceFromCorner
        if currentLine.intersects(labyrinthe_wall2) : 
            minDistanceFromCorner = min([currentLine.distance(Point(x)) for x in [(wall2["x"], wall2["y"]), (wall2["x"] + wall2["width"], wall2["y"]),
                                        (wall2["x"] + wall2["width"], wall2["y"] + wall2["height"]),
                                        (wall2["x"], wall2["y"] + wall2["height"])]])
            closeToWallMalus += malusGain + minDistanceFromCorner
...
def main():
    ...
    evolutionary_algorithm.variator = [inspyred.ec.variators.n_point_crossover,
    ...
    final_population = evolutionary_algorithm.evolve(
        ...
        num_crossover_points=1, #number of crossover point used
